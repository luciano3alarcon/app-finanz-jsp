		...
		if (aktionAbbrechen != null && aktionAbbrechen.equalsIgnoreCase("reset")) {
			...
			try {
				...
				if (id == null || id.isEmpty()) {
					...
					} else if (preis == null  ) { //preis.isEmpty() || 
						req.setAttribute("fehlerMeldungProdukt", "Der Preis darf nicht leer sein.");
						weiterGehen = false;
						
					} else if (anzahl.isEmpty()) {
						req.setAttribute("fehlerMeldungProdukt", "Die Anzahl darf nicht leer bleiben.");
						weiterGehen = false;
					}
					...
					} else if (Double.parseDouble(preis) == 0 || preis.isEmpty()) {
						req.setAttribute("fehlerMeldungProdukt", "Der Preis darf nicht leer sein.");
						weiterGehen = false;
					}
